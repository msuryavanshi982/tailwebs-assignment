...
const createTeacher = async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(data).some(a => a === "password")) {
      if (!password)
        return res.status(400).send({ status: false, message: "Enter Value in password Field " })

      if (!isValidPass(password))
        return res.status(400).send({ status: false, message: "Password must contain at least 8 characters including UPPER case, Number, Special Symbols" })
...
//=======================user/teacher Login Api=======================>>>>>

const teacherLoggedIn = async function (req, res) {
  try {
    let data = req.body;
...
module.exports = { createTeacher, teacherLoggedIn };
